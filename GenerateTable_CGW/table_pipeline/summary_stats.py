...
from .lc_lsp import resample_lc, normalize_flux
...
def add_variability_metrics(table, sigma_clip_thresh=None):
    """
    Add noise-corrected variability statistics to the table in-place,
    reading from the ``LC_flux`` and ``LC_flux_err`` columns.

    Parameters
    ----------
    table : pd.DataFrame
        Must have columns ``LC_t``, ``LC_flux``, ``LC_flux_err``.
    sigma_clip_thresh : float or None
